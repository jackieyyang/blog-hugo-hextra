...
(function () {
  ...
  async function preloadIndex() {
    ...
    const encodeBothEnZh = (str) => {
      const cjkParts = str.match(/[\u4e00-\u9fa5]+/g) || [];
      const nonCjkParts = str.split(/[\u4e00-\u9fa5]+/).filter(Boolean);
      return [...cjkParts, ...nonCjkParts.flatMap(part => 
        part.toLowerCase().split(/[\p{Z}\p{S}\p{P}\p{C}]+/u)
      )].filter(Boolean);
    };
